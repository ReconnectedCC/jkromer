...
public class ListNewestNames extends HttpEndpoint  {
    public static class ListNewestNamesBody extends ResponseBodyGeneric  {
        public Integer count;
        public Integer total;
        public List<Name> names;
    }

    public static CompletableFuture<Result<ListNewestNamesBody>> execute(Integer limit, Integer offset) {
        try {
            int useLimit = (limit == null) ? 50 : Math.min(Math.max(limit, 1), 1000);
            int useOffset = (offset == null) ? 0 : Math.max(offset, 0);

            String url = String.format("%s/names/newest?limit=%d&offset=%d",
                    endpoint,
                    useLimit,
                    useOffset
            );
    ...
    public static CompletableFuture<Result<ListNewestNamesBody>> execute() {
